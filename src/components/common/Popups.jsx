...
function Popups({ popup, close, language, mainTabs, loadMore, isLoading, gameRecord }) {
  return (
    <>
      ...
      <div className="overlay" style={{ visibility: popup.details ? "visible" : "hidden" }}>
        {popup.details ? <DetailsPopup close={close} mainTabs={mainTabs} /> : null}
      </div>
      <div className="overlay" style={{ visibility: popup.records ? "visible" : "hidden" }}>
        {popup.records ? <RecordsPopup mainTabs={mainTabs} close={close} loadMore={loadMore} isLoading={isLoading} gameRecord={gameRecord} /> : null}
      </div>
    </>
  );
}
